"""SQLAlchemy models for Contaspiccioli - Cash Flow Planner."""
import enum
...
class ForecastLine(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, index=True)
    forecast_month_id: Mapped[int] = mapped_column(ForeignKey("forecast_months.id"), nullable=False)
...
class Transaction(Base):
    ...
    category_id: Mapped[Optional[int]] = mapped_column(ForeignKey("categories.id"), nullable=True)
    forecast_line_id: Mapped[Optional[int]] = mapped_column(ForeignKey("forecast_lines.id"), nullable=True)
